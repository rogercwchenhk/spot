...
| 58 | 中国采购与招标网 | `requests_plain` | 174 | 0.44s | 首页li.news-info-item列表，标题完整 |
| 10 | 千里马招标网 | `requests_plain` | 94 | 0.15s | 首页混合布局，a[href*='/bid-']直接匹配 |
...
**千里马招标网**:
- 首页混合布局：表格(8条) + 列表(86条)
- 选择器：`a[href*="/bid-"]` 直接匹配所有bid链接
- 日期在表格 `td[3]` 或列表 `li > span` 中
- 详情页有419反爬（人机验证），暂不爬取详情

**中国采购与招标网**:
- 首页 `li.news-info-item` 列表，约217个item
- 标题在 `a[title]` 属性中（完整标题）
- 日期在 `span.time` 中（MM-DD格式）
- 过滤 `/bidDetail/` 链接排除新闻
- 详情页是SPA壳，暂不爬取详情
...
| 平台 | 结果 | 原因 |
|---|---|---|
| 标讯快车 (biaoxunkuaiche.com) | ❌ 不可用 | SSL证书过期 |
| 招标信息网 (bidnews.cn) | ❌ 不可用 | SPA超时（30s无响应） |
| 中国招标网 (zbcg.cn) | ❌ 不可用 | TLS错误，需大陆服务器 |
| 广东政府采购网 (gdgpo.czt.gd.gov.cn) | ⚠️ 需认证 | Vue SPA + Gateway，API需登录token |
